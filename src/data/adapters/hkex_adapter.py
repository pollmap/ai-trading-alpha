...
def _load_hkex_symbols() -> list[str]:
    """Load HKEX symbol list from config/markets.yaml with fallback to defaults."""
    try:
        with _MARKETS_YAML.open() as fh:
            config: dict[str, object] = yaml.safe_load(fh)
        hkex_section = config.get("HKEX", {})
        symbols: list[str] = hkex_section.get("symbols", [])  # type: ignore[union-attr]
        if symbols:
            return symbols
    except Exception as exc:
        log.warning("hkex_config_load_fallback", error=str(exc))
...
class HKEXAdapter(BaseMarketDataAdapter):
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._symbols: list[str] = _load_hkex_symbols()
        self._max_retries: int = LLM_MAX_RETRIES
        log.info(
            "hkex_adapter_initialized",
    ...
    def symbols(self) -> list[str]:
        """Return the configured HKEX symbol list."""
        return list(self._symbols)

    async def fetch_latest(self) -> dict[str, SymbolData]:
        """Fetch the latest OHLCV data for all configured HKEX symbols.
        ...
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                if not result:
                    msg = "yfinance returned data for zero HKEX symbols"
        ...
        msg = f"HKEX market data fetch failed after {self._max_retries} attempts"
