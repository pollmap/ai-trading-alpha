...
def _load_lse_symbols() -> list[str]:
    """Load LSE symbol list from config/markets.yaml with fallback to defaults."""
    try:
        with _MARKETS_YAML.open() as fh:
            config: dict[str, object] = yaml.safe_load(fh)
        lse_section = config.get("LSE", {})
        symbols: list[str] = lse_section.get("symbols", [])  # type: ignore[union-attr]
        if symbols:
            return symbols
    except Exception as exc:
        log.warning("lse_config_load_fallback", error=str(exc))
...
class LSEAdapter(BaseMarketDataAdapter):
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._symbols: list[str] = _load_lse_symbols()
        self._max_retries: int = LLM_MAX_RETRIES
        log.info(
            "lse_adapter_initialized",
    ...
    def symbols(self) -> list[str]:
        """Return the configured LSE symbol list."""
        return list(self._symbols)

    async def fetch_latest(self) -> dict[str, SymbolData]:
        """Fetch the latest OHLCV data for all configured LSE symbols.
        ...
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                if not result:
                    msg = "yfinance returned data for zero LSE symbols"
        ...
        msg = f"LSE market data fetch failed after {self._max_retries} attempts"
