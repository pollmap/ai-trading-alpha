...
def _load_jpx_symbols() -> list[str]:
    """Load JPX symbol list from config/markets.yaml with fallback to defaults."""
    try:
        with _MARKETS_YAML.open() as fh:
            config: dict[str, object] = yaml.safe_load(fh)
        jpx_section = config.get("JPX", {})
        symbols: list[str] = jpx_section.get("symbols", [])  # type: ignore[union-attr]
        if symbols:
            return symbols
    except Exception as exc:
        log.warning("jpx_config_load_fallback", error=str(exc))
...
class JPXAdapter(BaseMarketDataAdapter):
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._symbols: list[str] = _load_jpx_symbols()
        self._max_retries: int = LLM_MAX_RETRIES
        log.info(
            "jpx_adapter_initialized",
    ...
    def symbols(self) -> list[str]:
        """Return the configured JPX symbol list."""
        return list(self._symbols)

    async def fetch_latest(self) -> dict[str, SymbolData]:
        """Fetch the latest OHLCV data for all configured JPX symbols.
        ...
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                if not result:
                    msg = "yfinance returned data for zero JPX symbols"
        ...
        msg = f"JPX market data fetch failed after {self._max_retries} attempts"
