...
def _load_bond_symbols() -> list[str]:
    """Load BOND symbol list from config/markets.yaml with fallback to defaults."""
    try:
        with _MARKETS_YAML.open() as fh:
            config: dict[str, object] = yaml.safe_load(fh)
        bond_section = config.get("BOND", {})
        symbols: list[str] = bond_section.get("symbols", [])  # type: ignore[union-attr]
        if symbols:
            return symbols
    except Exception as exc:
        log.warning("bond_config_load_fallback", error=str(exc))
    ...
    for symbol in symbols:
        try:
            ...
            result[symbol] = SymbolData(
                ...
                currency="USD",
                per=None,   # Not applicable for bond ETFs
                pbr=None,   # Not applicable for bond ETFs
...
class BondAdapter(BaseMarketDataAdapter):
    """Global bond market data adapter using yfinance bond ETFs.
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._symbols: list[str] = _load_bond_symbols()
        self._max_retries: int = LLM_MAX_RETRIES
        log.info(
            "bond_adapter_initialized",
    ...
    def symbols(self) -> list[str]:
        """Return the configured bond symbol list."""
        return list(self._symbols)

    async def fetch_latest(self) -> dict[str, SymbolData]:
        """Fetch the latest OHLCV data for all configured bond ETFs.
        ...
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                if not result:
                    msg = "yfinance returned data for zero bond symbols"
        ...
        msg = f"Bond market data fetch failed after {self._max_retries} attempts"
